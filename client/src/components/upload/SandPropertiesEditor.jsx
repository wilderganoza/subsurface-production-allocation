  ...
  return (
    <div className="module-table-wrapper">
          ...
          <tbody>
            {rows.map((row, i) => (
              <tr key={i}>
                ...
                <td>
                  {rows.length > 1 && (
                    <button className="btn btn-sm btn-danger" type="button" onClick={() => removeRow(i)}>
                      Remove
                    </button>
                  )}
                </td>
      ...
      <div className="module-nav-buttons" style={{ marginTop: 12 }}>
        <button className="btn btn-sm btn-primary" type="button" onClick={addRow}>+ Add Sand</button>
      </div>
    </div>
  );
}
